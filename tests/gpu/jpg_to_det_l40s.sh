#!/usr/bin/env bash
#SBATCH --job-name=jpg_to_det_l40s
...
#SBATCH -o /project/dash_agir/brennen.farrell/logs-l40s/%x-%j.out
#SBATCH -e /project/dash_agir/brennen.farrell/logs-l40s/%x-%j.err
...
FINAL_DET_DIR="${FINAL_DET_DIR:-/90daydata/dash_agir/tmp/semifield-developed-images/new/l40s/detections}"
FINAL_VIZ_DIR="${FINAL_VIZ_DIR:-/90daydata/dash_agir/tmp/semifield-developed-images/new/l40s/visualizations}"
...
BATCH_ID="${BATCH_ID:-new-l40s}"

mkdir -p /project/dash_agir/brennen.farrell/logs-l40s
...
echo "Running JPG -> DET on L40S with model $MODEL_PATH..."
