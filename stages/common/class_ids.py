"""Resolve detection identities to the class IDs stored in uint8 masks."""
...
class DetectionBox:
    """A detection box plus its stable upstream row identity."""
    ...
    with csv_file:
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            if cultivar_id is not None and cultivar_id.strip():
                ...
                class_id = _parse_class_id(
                    entry["class_id"],
                    field=f"species {species_id!r} class_id",
                    context=context,
                )
